...
export KBC_CONFIG_ID="test-config"
...
curl -sS --fail -X POST \
  ...
  -d "name=test&configurationId=test-config"
printf "\nCreating configuration row" >&1
CONFIG_DATA=$(cat /code/Tests/sample-config.json)
COMMAND_RESULT=$(curl -sS --fail -X POST \
  {$KBC_STORAGEAPI_URL}/v2/storage/components/transformation/configs/test-config/rows \
  -H "content-type: application/x-www-form-urlencoded" \
  -H "x-storageapi-token: ${KBC_TOKEN}" \
